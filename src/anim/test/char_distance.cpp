...
TEST_CASE("Char construction and basic operations", "[char]") {
    ...
    SECTION("Constructor correctly initializes bitmap and index") {
        Char c(pattern2, 42);
        REQUIRE(std::memcmp(c.data(), pattern2, 8) == 0);
        // We don't have a getter for m_index, so we can't directly test it
    }
    ...
    SECTION("invert() correctly inverts all bits") {
        Char c(pattern2, 0); // 0xAA = 10101010
        c.invert();
...
TEST_CASE("Char distance calculation", "[char][distance]") {
    ...
    SECTION("Distance calculation is correct for one bit difference") {
        Char c1(pattern0, 0); // All zeros
        Char c2(pattern1, 1); // One bit set
        REQUIRE(c1.distance(c2) == 1);
        REQUIRE(c2.distance(c1) == 1);
    }

    SECTION("Distance calculation is correct for alternating patterns") {
        Char c1(pattern2, 2); // All 0xAA (10101010...)
        Char c2(pattern3, 3); // All 0x55 (01010101...)
    ...
    SECTION("Distance calculation is correct for specific patterns") {
        Char c1(pattern4, 4); // Pattern from pattern4
        Char c2(pattern5, 5); // Pattern from pattern5
    ...
    SECTION("Equality operator uses distance") {
        Char c1(pattern0, 0); // All zeros
        Char c2(pattern0, 0); // All zeros (same)
        Char c3(pattern1, 1); // One bit different
    ...
    SECTION("Distance function matches direct hamming_distance_8bytes call") {
        ...
        uint16_t direct_distance = hamming_distance_8bytes(
                c1.data(),  // Direct access to bitmap data
