...
namespace AnimTool {
    ...
    ANIM_TOOL_INLINE size_t fnv1a_hash(const std::string &str) { return fnv1a_hash(str.data(), str.size()); }

    ANIM_TOOL_INLINE uint16_t hamming_distance_8bytes(const unsigned char *bytes1, const unsigned char *bytes2) {
        uint16_t distance = 0;
        for (int i = 0; i < 8; i++) {
