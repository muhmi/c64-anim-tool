...
namespace AnimTool {
    ...
    class GifReader {
       public:
         ...
         */
        [[nodiscard]] static GifAnimation readAnimation(const std::string &filename,
                                                        const BitmapConverter &bitmapConverter);

       private:
        static GifFrame extractFrame(GifFileType *gif, int frameIndex, const BitmapConverter &paletteConverter);
